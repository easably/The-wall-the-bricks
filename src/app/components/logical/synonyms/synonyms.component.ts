...
export class SynonymsComponent implements OnInit {
  ...
  private wordsData: SynonymsData[];
  ...
  async ngOnInit() {
    const items = this.db.list('synonyms-words').valueChanges();
    this.wordsData = await items.pipe(take(1)).toPromise() as SynonymsData[]

    this.levelData = this.wordsData[this.currentLevel]
  }
  ...
  private nextLevel() {
    this.currentLevel++;
    if (this.wordsData[this.currentLevel]) {
